...
#### Bool Flags for btag, JER, other Nuisances ###
isBtag = False
isJER = False
isOther = False
isBinStat = True
...
if options.plotfile:
    ...
    canvas_nuis = ROOT.TCanvas("nuisancs", "nuisances", 1500, 1500)
    #canvas_nuis.SetBottomMargin(0.2)
    #canvas_nuis.SetTopMargin(0.2)
    print 'HERE'
    print options.ptol
    #if (options.ptol > 0.): 
    if (options.ptol > 0. or options.ptol == 0.0):
        nNuisCleaned = 0
        options.ptol = 0.001
        ...
        for ibin in range(1,hist_fit_b.GetNbinsX()+1):
            ...
            # Turn off specified nuisances
            #if '_stats_' in label: continue
            if isBinStat:
                if '_stats_' not in label: continue

            if isBtag:
                if 'btag' not in label: continue
                
            if isJER:
                if '_j_' not in label: continue

            if isOther:
                if '_j_' in label: continue
                if 'btag' in label: continue
                if '_stats_' in label: continue
        ...
        for ibin in range(1,hist_fit_b.GetNbinsX()+1):
            ...
            #if (label_b.find("_stats_") != -1): continue
            if isBinStat:
                if '_stats_' not in label_b: continue

            if isBtag:
                if 'btag' not in label_b: continue

            if isJER:
                if '_j_' not in label_b: continue

            if isOther:
                if '_j_' in label_b: continue
                if 'btag' in label_b: continue
                if '_stats_' in label_b: continue
            ...
            if ( (abs(theta_b/dtheta_b) > threshold) or (dtheta_b < (1-threshold)) ):
                ...
                hist_prefit_cleaned.GetXaxis().SetBinLabel(nSet,hist_prefit.GetXaxis().GetBinLabel(ibin).replace('CMS_vhbb_', ''))
