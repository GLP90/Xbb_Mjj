...
def rebinHist(hist, nbin, xmin, xmax, dirname, subdir, prefit_error_histos, postfit_error_histos, stat_hists):
  ...
  #       if b < 4 or b > 9:
#             h_new.SetBinContent(b, 0)
#             h_new.SetBinError(b,0)
#             h_new_postfit.SetBinContent(b, 0)
#             h_new_postfit.SetBinError(b,0)
...
#             print 'Post Fit Bin ',b, 'error:', hist.GetBinError(b)
    ...
    prefit_dirname = dirname.replace('postfit', 'prefit')
    print '\nSetting preFit Error(dirname,subdir)', prefit_dirname, subdir
    ROOT.gDirectory.cd(prefit_dirname)
...
def drawFromDC():
    ...
    ws_var = ROOT.RooRealVar(ws_var,ws_var,60,180)
    ...
    if 'Zll' in opts.bin or 'Zee' in opts.bin or 'Zuu' in opts.bin or 'Zmm' in opts.bin:
        ...
        setup = ['ZH', 'ggZH', 'VVHF', 'DY2b', 'DY1b', 'DYlight', 'TT','VVLF', 'ST']
        signalList = ['ZH']
        ...
        if 'med' in opts.bin or 'Med' in opts.bin: 
            pt_region_name = 'med'
            if lep_channel == 'Zmm':lep_channel = 'Zuu'
        
        # ['ZllHbb', 'ch1', 'Zmm', 'SIG', 'low']
        if 'Zmm' in lep_channel and 'low' in pt_region_name: zll_index = 'ch1'
        elif 'Zmm' in lep_channel and 'high' in pt_region_name: zll_index = 'ch3'
        elif 'Zee' in lep_channel and 'high' in pt_region_name: zll_index = 'ch4'
        elif 'Zee' in lep_channel and 'low' in pt_region_name: zll_index = 'ch2'
        else: zll_index = 'ch2'
    ...
    if 'Wmn' in opts.bin or 'Wen' in opts.bin:
        setup = ['WH', 'ZH', 'VVHF', 'Wj2b', 'Wj1b', 'Wj0b', 'DY2b', 'DY1b', 'DYlight', 'TT', 'VVLF', 'ST']
        signalList = ['ZH','WH']
        ...
        if 'Wen' in opts.bin: 
            ...
            elif 'wlf' in region: region_name = 'wlfWen'

            else: region_name = opts.bin
        ...
        if 'Wmn' in opts.bin:
            ...
            elif 'wlf' in region: region_name = 'wlfWmn'
    ...
    if 'Znn' in opts.bin:
        setup = ['ZH', 'ggZH', 'WH', 'VVHF', 'DY2b', 'DY1b', 'DYlight', 'Wj2b', 'Wj1b', 'Wj0b', 'TT', 'VVLF', 'ST']
        signalList = ['ZH']
    ...
    for dir in ROOT.gDirectory.GetListOfKeys():
        ...
        if 'Zuu' in opts.bin or 'Zee' in opts.bin:
            ...
            if 'med' in pt_region_name:
                if not (dirinfo[1] == lep_channel and dirinfo[3] == pt_region_name and dirinfo[4] == 'postfit'):
                    continue
                else: print '!!! Match !!!'
