...
def get_scale_factors(channel,labels,shift,v_b,input_sigma,nuisances):
    ...
    print '\n\tGetting SFs for Channel ' +  channel
    sf=[]
    sf_e=[]
...
for i in range(fpf_s.getSize()):
    ...
    if nuis_p == None:
        ...
        #mean_p, sigma_p = (nuis_p.getVal(), nuis_b.getError())
        ...
        if nuis_x == None:
            ...
            if nuis_p != None:
                ...
                if fit_name == 'b':
                    ...
                    if 'bin' in nuis_x.GetName():
                        bin_pulls.append(valShift)
                        bin_pull_names.append(nuis_x.GetName())
                    else:
                        pulls.append(valShift)
                        pull_names.append(nuis_x.GetName())
...
#                print sigShift
    ...
    for j in range(fpf_b.getSize()):
        ...
        if 'SF' in _name: continue
            
        if 'bin' in name or 'bin' in _name: continue
        
        nuis_count += 1
            
        print name + '  __CORR__  ' + _name 
            #print fit_b.correlation(name,_name)
            #print 'Error:', _nuis_b.getError()
        ...
        correlation_matrix_line.append(trunc(fit_b.correlation(name,_name),2))
            #print correlation_matrix_line


    if 'bin' not in name: 
        nuis1_count += 1

    print ' nuis_count:',  nuis_count
    print ' nuis1_count:',  nuis1_count
...
print 'correlation_matrix_name:', correlation_matrix_name
...
#del correlation_matrix[0]
print correlation_matrix

print 'Correlation Matrix x Length:', len(correlation_matrix)
print 'Correlation Matrix y Length:', len(correlation_matrix[0])
...
if True:
    ...
    histogram = ROOT.TH2F("Systematic Correlations", "", len(correlation_matrix), 1, len(correlation_matrix)+1, len(correlation_matrix), 1, len(correlation_matrix)+1)
    for x in range(0, len(correlation_matrix)):
        for y in range(0, len(correlation_matrix)):
            
            #print 'x, y:', x, y
            #print correlation_matrix[x][y]
            histogram.SetBinContent(x+1, y+1, float(correlation_matrix[x][y]))
            #histogram.GetXaxis().SetBinLabel(i+1, str(pull_names[i]))
            #histogram.Draw('pe')


    # Get the premade covariance matrix:
    #cov_b  = file.Get("covariance_fit_b")
    
    #print cov_b

    #histogram = cov_b

    canvas = ROOT.TCanvas("asdf", "", 1200, 1200)
    canvas.SetGrid()
    ...
    histogram.GetXaxis().SetLabelSize(0.02)
    histogram.GetYaxis().SetLabelSize(0.02)
    histogram.GetXaxis().SetNdivisions(24)
    histogram.GetYaxis().SetNdivisions(24)
    histogram.Draw('colz')
    #canvas.SaveAs(options.plotfile)
    canvas.SaveAs('/afs/cern.ch/user/d/dcurry/www/80x_validation/mlfit_plots/correlations_'+name_plot+'.png')
    canvas.SaveAs('/afs/cern.ch/user/d/dcurry/www/80x_validation/mlfit_plots/correlations_'+name_plot+'.pdf')
...
if True:
    ...
    histogram = ROOT.TH1F("pulls", "Pulls", len(pulls), 0, len(pulls)+1)
    for i in range(0, len(pulls)):
        histogram.Fill(i+1, pulls[i])
        histogram.GetXaxis().SetBinLabel(i+1, str(pull_names[i]))
        histogram.Draw('pe')

    canvas = ROOT.TCanvas("asdf", "asdf", 1200, 800)
    canvas.SetGrid()
    ...
    histogram.GetXaxis().SetLabelSize(0.02)
    histogram.GetXaxis().SetNdivisions(-len(pulls));
    #histogram.Fit("gaus")
    histogram.Draw("p")
    #canvas.SaveAs(options.plotfile)
    canvas.SaveAs('/afs/cern.ch/user/d/dcurry/www/80x_validation/mlfit_plots/pulls_'+name_plot+'.png')
    canvas.SaveAs('/afs/cern.ch/user/d/dcurry/www/80x_validation/mlfit_plots/pulls_'+name_plot+'.pdf')
    
    histogram = ROOT.TH1F("pulls", "Pulls", 60, -2, 2)
    for pull in pulls:
        histogram.Fill(pull)
    for pull in bin_pulls:
        histogram.Fill(pull)
    ...
    histogram.Draw("pe")
    #canvas.SaveAs(options.plotfile)
    canvas.SaveAs('/afs/cern.ch/user/d/dcurry/www/80x_validation/mlfit_plots/pulls_gaus_'+name_plot+'.png')
    canvas.SaveAs('/afs/cern.ch/user/d/dcurry/www/80x_validation/mlfit_plots/pulls_guas_'+name_plot+'.pdf')
...
#CMS_vhbb_ZjLF_Zll_SF_8TeV                     ! +3.51, 0.94!     ! +3.42, 0.95!       -0.00
    ...
    for channel,active in ch.iteritems():
        ...
        if active > 0.:
            ...
            print '----> Getting Scale Factors1...'
            sf = get_scale_factors(channel,labels,shift,v_b,input_sigma,nuisances)[0]

            print '----> Getting Scale Factors2...'
            sf_e = get_scale_factors(channel,labels,shift,v_b,input_sigma,nuisances)[1]
