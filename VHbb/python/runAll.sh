...
if [ $task = "prep" ]; then
    ./prepare_environment_with_config.py --samples $sample --config ${energy}config/${configList} --config ${energy}config/samples_nosplit.cfg  
fi
if [ $task = "trainReg" ]; then
    ./trainRegression.py --config ${energy}config/${configList} --config ${energy}config/regression
fi
if [ $task = "sys" ]; then
   # ./writeRegZll.py --samples $sample --config ${energy}config/${configList} --config ${energy}config/regression
    ./write_regression_systematics.py --samples $sample --config ${energy}config/${configList} --config ${energy}config/regression
fi
if [ $task = "evalBDT" ]; then
    ./evaluateMVA.py --discr $MVAList --samples $sample --config ${energy}config/${configList}
fi

if [ $task = "plot_sys" ]; then
    ./plot_systematics.py --config ${energy}config/${configList} --config ${energy}config/datacard   
fi

if [ $task = "plot_batch" ]; then
    ./tree_stack.py --region $sample --config ${energy}config/${configList} --config ${energy}config/configPlot_vars  --config ${energy}config/regression --batch True
fi

if [ $task = "plotDC" ]; then
    ./stack_from_dc.py --config ${energy}config/${configList} --config ${energy}config/datacard
fi
...
if [ $task = "trainBDT" ]; then
    ./train.py --training $sample --config ${energy}config/${configList} --local True
fi
if [ $task = "plot" ]; then
    ./tree_stack.py --region $sample --config ${energy}config/${configList} --config ${energy}config/configPlot_vars  --config ${energy}config/regression
fi
if [ $task = "dc" ]; then
    ./workspace_datacard.py --variable $sample --config ${energy}config/${configList} --config ${energy}config/datacard
fi
if [ $task = "split" ]; then
    ./split_tree.py --samples $sample --config ${energy}config/${configList} --max-events $job_id
fi
if [ $task = "kinFit" ]; then
    ./kinFit_Hmass.py --samples $sample --config ${energy}config/${configList} --config ${energy}config/regression
fi 
